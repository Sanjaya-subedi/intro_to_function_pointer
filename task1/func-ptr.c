...
{
	//TODO: IMPLEMENT ME!
	Process *first_p = (Process *) this;
	Process *second_p = (Process *) that;

	if (first_p -> priority < second_p -> priority){
		return 1;
	}else if (first_p -> priority > second_p -> priority){
		return -1;
	} else{
		if (first_p -> arrival_time < second_p -> arrival_time ){
			return -1;
		}else if (first_p -> arrival_time > second_p -> arrival_time ){
			return 1;
		}
	}
